...
void RegisterSubWindowPlugins(flutter::PluginRegistry* registry) {
  ...
  // media_kit_video uses a process-wide singleton and replaces the root window
  // proc on registration, which crashes when a second Flutter engine registers
  // inside desktop_multi_window sub-windows.
...
bool FlutterWindow::OnCreate() {
  ...
  DesktopMultiWindowSetWindowCreatedCallback([](void* controller) {
    ...
    // desktop_multi_window already performs its own internal registration for
    // sub-window engines; skip full registration here to avoid re-attaching
    // "main window" and breaking multi-window channel stability.
    RegisterSubWindowPlugins(flutter_view_controller->engine());
  });
