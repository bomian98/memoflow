...
/// Window attribute that controls corner rounding preference.
///
/// Redefined for older Windows SDKs.
...
constexpr DWORD kDwmWindowCornerRound = 2;  // DWMWCP_ROUND
constexpr int kFallbackCornerRadius = 14;
...
void UpdateRoundedCorners(HWND const window) {
  HRESULT result = DwmSetWindowAttribute(
      window, DWMWA_WINDOW_CORNER_PREFERENCE, &kDwmWindowCornerRound,
      sizeof(kDwmWindowCornerRound));
  if (SUCCEEDED(result)) {
    DwmSetWindowAttribute(window, DWMWA_BORDER_COLOR, &kDwmColorNone,
  ...
  HRGN rounded = CreateRoundRectRgn(0, 0, width + 1, height + 1,
                                    kFallbackCornerRadius,
                                    kFallbackCornerRadius);
  if (rounded != nullptr) {
    SetWindowRgn(window, rounded, TRUE);
  }
}
